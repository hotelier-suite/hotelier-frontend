      ...
      <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
        ...
        <p className="text-sm text-gray-600">
          ...
          <span className="font-medium">
            {data.value} {data.labelEmployees}
          </span>
        </p>
        <p className="text-sm text-green-600">
          ✓ {data.activeCount} {data.labelActive} ({activePercentage}%)
        </p>
        {data.inactiveCount > 0 && (
          <p className="text-sm text-gray-500">
            ○ {data.inactiveCount} {data.labelInactive}
          </p>
        )}
      </div>
    );
...
export function EmployeesPieChart({ departmentStats }: EmployeesPieChartProps) {
  const t = useTranslations("EmployeesPieChart");
  const chartData = departmentStats
    .filter((dept) => dept.totalCount > 0)
    .map((dept) => ({
      name: DEPARTMENT_KEYS[dept.department]
        ? t(DEPARTMENT_KEYS[dept.department])
        : dept.department,
      value: dept.totalCount,
      activeCount: dept.activeCount,
      inactiveCount: dept.totalCount - dept.activeCount,
      labelTotal: t("tooltipTotal"),
      labelEmployees: t("tooltipEmployees"),
      labelActive: t("tooltipActive"),
      labelInactive: t("tooltipInactive"),
    }));
