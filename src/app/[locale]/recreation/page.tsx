...
export default function RecreationPage() {
  const t = useTranslations("RecreationPage");
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {t("ofFacilities", { count: facilities.length })}
            </p>
          </CardContent>
        </Card>
              ...
              >
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {facility.status === "AVAILABLE" && t("statusAvailable")}
                      {facility.status === "OCCUPIED" && t("statusOccupied")}
                      {facility.status === "MAINTENANCE" &&
                        t("statusMaintenance")}
                      {facility.status === "OUT_OF_ORDER" &&
                        t("statusOutOfService")}
                      {facility.status === "RESERVED" && t("statusReserved")}
                      {facility.status === "CLEANING" && t("statusCleaning")}
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent>
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground mt-1">
                    ...
                    <span>{t(`facilityTypes.${facility.type}`)}</span>
                  </div>
                </CardContent>
              </Card>
            ))}
