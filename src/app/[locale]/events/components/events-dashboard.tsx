...
export default function EventsDashboard({
  ...
  const handleEventAdd = async (eventData: ApiEventBooking) => {
    try {
      ...
      toast.error(t("eventCreatedError"));
    }
  };
  ...
  const handleEventUpdate = async (
    ...
    try {
      ...
      toast.error(t("eventUpdatedError"));
    }
  };
  ...
  const handleEventDelete = async (eventId: number) => {
    try {
      ...
      toast.error(t("eventDeletedError"));
    }
  };
    ...
    <div className="w-full max-w-full space-y-6 overflow-hidden">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {t("ofVenues", { count: venues.length })}
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatCurrency(events.reduce((total) => total + 0, 0))}
            </div>
      ...
      <Tabs defaultValue="events" className="w-full space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="salones">{t("venuesTab")}</TabsTrigger>
          <TabsTrigger value="calendario">{t("calendarTab")}</TabsTrigger>
        </TabsList>
