...
import { NewReservationDialog } from "@/app/[locale]/reservations/components/new-reservation-dialog";
...
export function MyReservationsManagement() {
  const t = useTranslations("MyReservationsComp");
  ...
  const handleCreateMyReservation = async (formData: {
    ...
    try {
      ...
      toast.success(t("reservationCreated"), {
        description: t("reservationCreatedDesc"),
      });
    } catch (e) {
      console.error("Error creating self reservation:", e);
      toast.error(t("error"), {
        description: t("couldNotCreate"),
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>{t("reservationsList")}</CardTitle>
        ...
        <CardContent>
          {loading ? (
            <CardLoader text={t("loadingReservations")} />
          ) : filteredReservations.length === 0 ? (
            <EmptyState
              icon={CalendarPlus}
              title={searchTerm ? t("noResults") : t("noReservations")}
              description={
                searchTerm
                  ? t("noReservationsMatchSearch")
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="w-24">{t("room")}</TableHead>
                          ...
                          {r.companions && r.companions.length > 0 ? (
                            <div>
                              <div className="font-medium">
                                {1 + r.companions.length} {t("guestsCount")}
                              </div>
                              <div className="text-muted-foreground">
                                <div>1 {t("principal")}</div>
                                {r.companions.slice(0, 2).map((c, i) => (
                                  <div key={i}>{c.name}</div>
                                ))}
                                {r.companions.length > 2 && (
                                  <div>
                                    +{r.companions.length - 2} {t("more")}
                                  </div>
                                )}
                              </div>
                            </div>
                        ...
                        <div className="text-sm">
                          {r.discountPercent || r.discountAmount ? (
                            <div>
                              {r.discountPercent && (
                                <div>
                                  {r.discountPercent}% {t("discount")}
                                </div>
                              )}
                              {r.discountAmount && (
                                <div>
                                  {formatCurrencyAmount(r.discountAmount)}{" "}
                                  {t("fixed")}
                                </div>
                              )}
                            </div>
